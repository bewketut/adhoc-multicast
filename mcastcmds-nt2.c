...
if((so[channel][fileindex]=socket(AF_INET, SOCK_DGRAM,0))<0) exit(0);
temp[channel][fileindex].sin_family=AF_INET;
temp[channel][fileindex].sin_addr.s_addr=inet_addr("127.0.0.1");
//temp[channel][fileindex].sin_port=htons(30100);
//else
temp[channel][fileindex].sin_port=htons(3100+channelport);
}
else if (!(fn[channel][fileindex]=fopen(channelfolder,"w"))){
...
if(!(fn[channel][fileindex]=fopen(channel4all,"w")) && fopen("ttt.t","w")){
...
fn[channel][fileindex]=fopen(message+6,"w");
...
fileindex=((unsigned char) message[MCASTBUF_SIZ-1])%NMUTEXFILES;
channelport= ((channel-'0') > 0)?channel-'0': channel;
 snprintf(channelfolder+7,4,"%d",channelport);
strcat(channelfolder,"/");
